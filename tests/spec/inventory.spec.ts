...
test.describe("Inventory Scenarios:", () => {
	...
	for (const inventoryOrderScenario of inventoryOrderScenarios) {
		test(`Inventory Order Scenario: ${inventoryOrderScenario.testCase}`, async ({
			page,
		}) => {
			const inventoryPage = new InventoryPage(page);
			await inventoryPage.selectProductSortBy(
				inventoryOrderScenario.inventory_order,
			);
			await inventoryPage.assertFirstElementHasHeading(
				inventoryOrderScenario.heading,
			);
		});
	}
